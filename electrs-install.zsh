...
install_build_dependencies() {
    ...
    if ((1)); then
        ...
        if ! program_exists "rustup"; then
            ...
            pv_exec -l "Running rustup update..." \
                rustup update \
            || exit $?
        fi
    fi
...
build_electrs() {
    ...
    if ((0)); then
        ...
        pv_exec -l "Running cargo clean..." \
            cargo clean \
        || success=0
        if (( success )); then
            pv_exec -l "Running cargo update..." \
                cargo update \
            || success=0
        fi
    fi
    if (( success )); then
        ...
        pv_exec -l "Running cargo build..." -o "$BUILD_LOGS_DIR/electrs-cargo-build.log" \
...
# next time the electrs-install.zsh script is executed. To change any configuration settings, edit
# the file:
#
...
#
#  ${SCRIPT_DIR}/electrs-install.zsh
#
############################
